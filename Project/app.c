...
void destroy(void);
int findGTK(char *buffName, struct buff *buffers, int buffCount);
...
int main(int argc, char **argv)
{
	...
	while (1)
	{
		...
		else if (strcmp(command, "display") == 0)
		{
			...
			GtkWidget *window;
			int num = findGTK(buffName, buffers, buffCount);
			gtk_init(&argc, &argv);
			window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
			// image = gtk_image_new_from_file(tem.imageName);
			// image = gtk_image_new_from_pixbuf(tem.pixImg);
			gtk_signal_connect(GTK_OBJECT(window), "destroy",
							   GTK_SIGNAL_FUNC(destroy), NULL);

			gtk_container_add(GTK_CONTAINER(window), gtk[num]);
